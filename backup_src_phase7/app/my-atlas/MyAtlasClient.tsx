...
export default function MyAtlasClient({
    ...
    const [selectedArtist, setSelectedArtist] = useState<string | null>(bookmarks.length > 0 ? bookmarks[0].name : null);
    ...
    return (
        <main className="flex-1 flex flex-col lg:flex-row overflow-hidden relative" style={{ height: "calc(100vh - 56px)" }}>
                    ...
                    {bookmarks.length === 0 ? (
                        ...
                        bookmarks.map((b) => {
                            ...
                            return (
                                <button
                                    key={b.id}
                                    onClick={() => setSelectedArtist(b.name)}
            ...
            {/* Right Content */}
            <div className={`flex-1 relative bg-[#FAFAFA] min-h-[500px] lg:min-h-0 overflow-hidden transition-all duration-300 ease-in-out`}>

                {selectedArtist ? (
                    <div className="absolute inset-0 flex flex-col bg-white">
                        <ArtistDrawer
                            artistName={selectedArtist}
                            onClose={() => { }} // Disabled close button behavior since it's the main view now
